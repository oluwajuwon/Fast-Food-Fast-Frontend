...
describe('login reducer', () => {
  ...
  it('returns the correct state', async () => {
    ...
    const payload = {
      success: 'true', message: 'login successful', username: 'jayboy',
    };
    await axiosMock.onPost().reply(200, payload);
    const action = { type: 'LOG_IN', payload };
    const expectedState = { isLoggedin: true, response: {}, user: { success: 'true', message: 'login successful', username: 'jayboy' } };
